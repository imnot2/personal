...
(function (j$) {
    j$.fn.jChinaArea = function (o) {
        ...
        function writeInput() {
            if (o.txtOrCode) {
                var sProvinceTxt = j$(":selected", sProvince).text();
                if (sProvinceTxt == "无") sProvinceTxt = "";
                var tCityTxt = j$(":selected", sCity).text();
                if (tCityTxt == "无") tCityTxt = "";
                var tCountyTxt = j$(":selected", sCounty).text();
                if (tCountyTxt == "无") tCountyTxt = "";
                tProvince.val(sProvinceTxt);
                tCity.val(tCityTxt);
                tCounty.val(tCountyTxt);
            } else {
                tProvince.val(j$(":selected", sProvince).val());
                tCity.val(j$(":selected", sCity).val());
                tCounty.val(j$(":selected", sCounty).val());
            }
        }
    };
})(jQuery);
